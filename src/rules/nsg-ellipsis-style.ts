 ...
 * 偽陽性回避:
 *   - 半角ピリオド3個以上の連続に限定する。
 *     2個（..）は相対パス記法や TypeScript の型記法と区別しにくいため対象外。
 *   - パス表記「../」を避けるため、直後がスラッシュの場合は対象外（lookahead）。
 *   - JavaScript スプレッド構文「...」や英語省略「etc...」は3個に当たるが、
 *     日本語文書校正用途では severity=warning（提案）に留めることで許容する。
 */
import type {
  LintIssue,
  LintRule,
...
// 連続3個以上の半角ピリオド。直後がスラッシュ（相対パス ../）の場合は除外。
const PATTERN = /\.{3,}(?!\/)/;
